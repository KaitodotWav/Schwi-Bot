...
from KaitoUWU import Links as Klink
...
class MCServersMonitor(commands.Cog):
    ...
    async def monitor(self, ctx, option, *args):
        add_EMB = discord.Embed(title="Processing", description="please wait...", color=0xFF0000)
        add_EMB.set_thumbnail(url = str(self.ico["mumei_loading1"]))
        MEMB = await ctx.send(embed=add_EMB)
        EMBico = Rimage()
        ...
        async def unknown_args(MEMB):
            res_add_EMB = discord.Embed(title="Unknown Arguement", description="my brain cant process this command.", color=0x964B00)
            res_add_EMB.set_thumbnail(url=self.ico["mumei_think1"])
            await self.EditEMB(MEMB, res_add_EMB)
        ...
        if str(option) == "add":
            if str(args[0]).lower() == "server":
                ...
                try:
                    ...
                    platform = "java"
                await add_srv(ctx.channel.id, ip, platform)
            else:
